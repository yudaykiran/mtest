...
cd /opt/gopath/src/github.com/openebs/mtest

# Install dependencies required for Development
bash buildscripts/install_go.sh

# CD into the mtest working directory when we login to the VM
# A bit of conditional logic s.t. we do not repeat CD-ing
grep "cd /opt/gopath/src/github.com/openebs/mtest" /home/vagrant/.profile || \
  echo "cd /opt/gopath/src/github.com/openebs/mtest" >> /home/vagrant/.profile
...
def configureVM(vmCfg, hostname, cpus, mem)
  ...
  vmCfg.vm.synced_folder '.', '/opt/gopath/src/github.com/openebs/mtest'
